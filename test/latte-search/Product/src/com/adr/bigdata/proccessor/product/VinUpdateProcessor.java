...
public class VinUpdateProcessor extends UpdateRequestProcessor {
	private static final Logger log = LoggerFactory.getLogger(VinUpdateProcessor.class);
	...
	private static final String ATT_IS_SEARCH = "is_search";
	private static final String ATT_IS_INTERNAL_TAG = "is_internal_tag";
	private static final String CONFIG_FILE_PARAM = "attSearchFileName";

	@SuppressWarnings({ "unchecked", "rawtypes" })
	VinUpdateProcessor(SolrParams parameters, SolrQueryRequest request, SolrQueryResponse response,
	...
	public void processIndexAttribute(AddUpdateCommand command) {
		...
		if (!searchedAttributes.isEmpty()) {
			...
			List<String> internalTagValues = new ArrayList<String>();
			float weight = 1;
			for (CategoryAttributeConfig attConfig : searchedAttributes) {
				...
				String attributeValueIgnore = (String) document.getFieldValue("attr_" + attConfig.getAttributeId() + "_txt_ignore");
				attributeValue = !Strings.isNullOrEmpty(attributeValue) ? attributeValue : attributeValueIgnore; 

				if (!Strings.isNullOrEmpty(attributeValue) && attConfig.isSearch()) {
					searchValues.add(attConfig.getAttributeName() + attributeValue);
					weight = attConfig.getWeight();
				}
				
				if (!Strings.isNullOrEmpty(attributeValue) && attConfig.isInternalTag()) {
					internalTagValues.add(attConfig.getAttributeName() + attributeValue);
					weight = attConfig.getWeight();
				}
			}

			if (!searchValues.isEmpty()) {
				document.setField("attribute_search", searchValues, weight);
			}
			
			if (!internalTagValues.isEmpty()) {
				document.setField("internal_tags", searchValues, weight);
			}
		}
	}
	...
	private void loadConfig(JSONObject o, Map<Integer, Map<Integer, CategoryAttributeConfig>> config) {
		...
		for (Object obj : attributes) {
			...
			boolean isSearch = (boolean) attribute.get(ATT_IS_SEARCH);
			boolean isInternalTag = (boolean) attribute.get(ATT_IS_INTERNAL_TAG);
			
			CategoryAttributeConfig caConfig = new CategoryAttributeConfig(catId, attId, attName, attWeight, isSearch, isInternalTag);
			if (config.containsKey(catId)) {
