package com.adr.bigdata.search.wt;
...
public class NoShardingMappingJsonWriter implements QueryResponseWriter, Loggable {
	...
	public void write(Writer writer, SolrQueryRequest req, SolrQueryResponse rsp) throws IOException {
		// special process
		if (req.getParams().getBool("vincache", false)) {
			String cache = (String) rsp.getValues().get("vincache");
			writer.write(cache);
		} else {		
				...
				if (mapped.equals("response.docs")) {
					if (rsp.getValues().get("response") != null) {
						...
						if (response instanceof ResultContext) {
							map.put(path, ((ResultContext) response).docs);
						} else if (response instanceof SolrDocumentList) {
							map.put(path, ((SolrDocumentList) response));
						}					
					}
					...
					if (rsp.getValues().get("response") != null) {
						Object response = rsp.getValues().get("response");
						if (response instanceof ResultContext) {
							map.put(path, ((ResultContext) response).docs.matches());
						} else if (response instanceof SolrDocumentList) {
							map.put(path, ((SolrDocumentList) response).getNumFound());
						}					
					}
					...
					if (rsp.getValues().get("response") != null) {
						Object response = rsp.getValues().get("response");
						if (response instanceof ResultContext) {
							map.put(path, ((ResultContext) response).docs.offset());
						} else if (response instanceof SolrDocumentList) {
							map.put(path, ((SolrDocumentList) response).getStart());
						}					
					}
				}
				// End: Hard code ;))
...
@SuppressWarnings("rawtypes")
class JSONWriter extends TextResponseWriter implements Loggable {
	...
	public JSONWriter(Writer writer, SolrQueryRequest req, SolrQueryResponse rsp) {
		...
		jsonSuffix = req.getParams().get("vin_suffix", "_json");
	}
	...
	public void writeSolrDocument(String name, SolrDocument doc, ReturnFields returnFields, int idx)
		...
		for (String fname : doc.getFieldNames()) {
			...
			if (val instanceof List) {
				...
				if (fname.endsWith(this.jsonSuffix) && val instanceof Field) {
					this.writer.write(((Field)val).stringValue());
				} else {
					writeVal(fname, val);
				}
			}
