...
import static com.adr.bigdata.indexingrd.models.impl.CommisionModel.C_CUSTOM_COMMISION_FEE;
import static com.adr.bigdata.indexingrd.models.impl.CommisionModel.C_MERCHANT_ID;
...
import com.adr.bigdata.indexingrd.models.impl.CommisionModel;
	...
	public void execute(PuObject data) throws Exception {
		...
		if (merchantId == null || customCommisionFee == null) {
			...
			customCommisionFee.forEach(commision -> {
				Integer catId = commision.getInteger(CommisionModel.C_CATGORY_ID);
				Integer isNotApplyCommision = commision.getInteger(CommisionModel.C_IS_NOT_APPLY_COMMISION);
				Integer commisionFee = commision.getInteger(CommisionModel.C_COMMISION_FEE);
				Long updateTime = commision.getLong(CommisionModel.C_UPDATE_TIME);
				if (catId == null || isNotApplyCommision == null || commisionFee == null || updateTime == null) {
					...
					CommisionVO commsionVO = new CommisionVO(merchantId, catId, isNotApplyCommision, commisionFee, updateTime);
					mapData.put(merchantId + "_" + catId, commsionVO);
				}
			});
			Map<String, String> lastUpdateTime = model.getLastUpdateTimes(merchantId);
			List<Integer> processedCat = new ArrayList<Integer>();
			mapData.forEach((mcCat, commision) -> {
				if (lastUpdateTime.containsKey(mcCat) && Long.valueOf(lastUpdateTime.get(mcCat)) >= commision.getUpdateTime()) {
			...
			model.updateToRedis(processedData);
			model.updateTimeToCache(merchantId, catIdUpdateTimes);
			model.sendLastUpdatedWhpIdsInteger(catIdUpdateTimes.keySet());
